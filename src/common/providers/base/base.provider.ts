...
export abstract class BaseProvider<TCredentials extends IProviderCredentials> 
  ...
  protected isRetryableError(error: Error): boolean {
    // Can be overridden by specific providers
    const retryablePatterns = ['ECONNRESET', 'ETIMEDOUT', 'RATE_LIMIT', '429', '503', '504'];
  ...
  protected logSendAttempt(payload: ProviderSendPayload): void {
    this.logger.log(
      `Sending via ${this.getProviderName()} to ${
        payload.recipient.email || payload.recipient.phone || payload.recipient.deviceToken
      }`
    );
  }
