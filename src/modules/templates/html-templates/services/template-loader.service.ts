...
export class TemplateLoaderService implements OnModuleInit {
  ...
  private async loadPartials(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(`Could not initialize partials: ${(error as Error).message}`);
    }
  }
  ...
  private async registerPartialsFromDirectory(
    ...
    try {
      ...
      this.logger.warn(`Could not load partials from ${dir}: ${(error as Error).message}`);
    }
  }
  ...
  async loadTemplate(templatePath: string): Promise<string> {
    ...
    try {
      ...
      this.logger.error(`Failed to load template ${templatePath}: ${(error as Error).message}`);
  ...
  private async loadTemplatesFromDirectory(
    ...
    try {
      ...
      this.logger.warn(`Could not load templates from ${dir}: ${(error as Error).message}`);
    }
  }
}
